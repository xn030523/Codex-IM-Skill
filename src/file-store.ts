/**
 * 基于 JSON 文件的 BridgeStore 实现。
 *
 * 功能入口：
 * - 管理会话、聊天绑定、消息历史、权限链接、去重键等 bridge 状态。
 * 输入输出：
 * - 输入来自 bridge 生命周期中的各种查询与写入调用。
 * - 输出为 ~/.codex-skill/data/ 下的多个 JSON 文件。
 * 边界与异常：
 * - 文件不存在时回退为空数据；该实现适合单机轻量场景，不保证多进程并发写入安全。
 */
...
function atomicWrite(filePath: string, data: string): void {
  // 关键逻辑：所有落盘都走原子替换，减少异常退出导致 JSON 损坏的概率。
...
export class JsonBridgeStore implements BridgeStore {
  ...
  constructor(settingsMap: Map<string, string>) {
    ...
    // 功能入口：构造阶段加载索引型数据，后续消息正文按 session 懒加载。
    this.loadAll();
  }
  ...
  private loadAll(): void {
    // 关键逻辑：启动时先恢复轻量索引数据，避免每次都把全部消息历史一次性读入内存。
    // Sessions
  ...
  private loadMessages(sessionId: string): BridgeMessage[] {
    ...
    // 关键逻辑：消息历史按会话单独存储并懒加载，降低冷启动 I/O 成本。
  ...
  upsertChannelBinding(data: UpsertChannelBindingInput): ChannelBinding {
    ...
    if (existing) {
      // 关键逻辑：同一个 chat 复用原绑定，只更新会话、目录、模型等动态信息。
    ...
    const binding: ChannelBinding = {
      ...
      workingDirectory: data.workingDirectory,
      model: data.model,
      // 默认 mode 来自配置；缺失时退回 code，保证新会话具备可执行能力。
      mode:
  ...
  createSession(
    ...
    const session: BridgeSession = {
      id: uuid(),
      working_directory:
        cwd || this.settings.get("bridge_default_work_dir") || process.cwd(),
      model,
  ...
  acquireSessionLock(
    ...
    if (existing && existing.expiresAt > Date.now()) {
      // 边界与异常：锁未过期且 lockId 不同，说明已有任务在处理该会话，不允许并发占用。
  ...
  updateSdkSessionId(sessionId: string, sdkSessionId: string): void {
    const s = this.sessions.get(sessionId);
    if (s) {
      // 关键逻辑：session 与 binding 要同时更新，恢复会话时才能拿到一致的 SDK 会话 ID。
      (s as unknown as Record<string, unknown>)["sdk_session_id"] =
        sdkSessionId;
      this.persistSessions();
    }
    // 关键逻辑：历史绑定也需要同步更新，否则 IM 聊天恢复时会丢失 provider 上下文。
    for (const [key, b] of this.bindings) {
  ...
  insertAuditLog(entry: AuditLogInput): void {
    ...
    // 边界与异常：审计日志只保留最近 1000 条，避免长期运行后文件无限膨胀。
  ...
  checkDedup(key: string): boolean {
    ...
    // 边界与异常：去重窗口固定为 5 分钟，超时后自动失效，避免误伤正常重复消息。
